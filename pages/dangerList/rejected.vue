	...
	<view class="rejected">
		...
		<u--form class="rejected-form" labelPosition="left" :model="userAdd" :rules="rules" ref="uForm">
			<u-form-item class="form-item" prop="name" @click="show = true" borderBottom>
				...
				<u--input v-model="userAdd.name"   placeholder="整改不合格，请按规范整改" border="none"></u--input>
			</u-form-item>
...
export default {
	...
	methods: {
		handReview(){
			...
			this.$http(`/problems/reject/${this.objId.id}`, 'POST', {
				recheckDesc:this.userAdd.name,
				recheckImages:this.userAdd.images
			}, false)
				.then(res => {
					console.log(res)
					uni.hideLoading();
